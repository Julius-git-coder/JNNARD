...
export function SelectTrigger({ className, children, title, tooltip }: { className?: string; children: React.ReactNode; title?: string; tooltip?: string }) {
    ...
    const trigger = (
        <button
            ...
            title={title}
            className={cn(
                "flex h-10 w-full items-center justify-between rounded-md border border-gray-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-50 dark:ring-offset-gray-950 dark:placeholder:text-gray-400",
                className
            )}
        >
            {children}
